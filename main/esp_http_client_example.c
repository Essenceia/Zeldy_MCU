...
static void http_test_teapot_task(void *pvParameters)
{
    app_wifi_wait_connected();
    char *buffer = malloc(MAX_HTTP_RECV_BUFFER + 1);
    if (buffer == NULL) {
        ESP_LOGE(TAG, "Cannot malloc http teapot receive buffer");
        return;
    }
...
void app_main()
{
   ...
   // xTaskCreate(&http_test_teapot_task, "http_test_teapot_task", 8192, NULL, 5, NULL);
  //  xTaskCreate(&http_test_task, "http_test_task", 8192, NULL, 5, NULL);

    for(int i = 0 ; i < 100 ; i ++ ){
        ESP_LOGE("ADC","[%d] Value read %d : voltage %d ",i, get_ADC(),read_voltage());
